...
class ViewProducts(StoredProcedureTestCase):
    def test_view_products(self):
        ...
        fetchedProducts = view_products(db=self.db,
                                                productCategoryId=productCategory1["product_category_id"],
                                                sortOrder="ascending")
...
def view_products(db, productCategoryId, sortOrder=None):
    db.call_procedure("ViewProducts", [productCategoryId, sortOrder])
    results = []
    for row in db:
        result = {
            "product_id": row["product_id"],
