...
class FilterProducts(StoredProcedureTestCase):
    ...
    def test_filter_products(self):
        ...
        filteredProducts = filter_products(db=self.db,
                                                    filterText="row",
                                                    sortOrder="ascending",
                                                    productCategoryId=productCategory2["product_category_id"])
...
def filter_products(db, filterText, sortOrder, productCategoryId):
    db.call_procedure("FilterProducts", [filterText, sortOrder, productCategoryId])
    results = []
    for row in db:
        result = {
            "product_id": row["product_id"],
